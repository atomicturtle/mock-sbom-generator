...
# mock-sbom-generator 1.x plugin keys. Distro Mock does not seed ``command``,
# so existing Atomicorp configs still pass these booleans on
# sbom_generator_opts. Mapped onto DEFAULT_COMMAND only when ``command`` is
# unset; an explicit ``command`` always wins.
_LEGACY_INCLUDE_DEFAULTS = (
...
_LEGACY_INCLUDE_KEYS = tuple(name for name, _default in _LEGACY_INCLUDE_DEFAULTS)


def _legacy_bool(value, default):
    """Coerce a 1.x plugin opt to bool without raising on junk values."""
...
def _command_from_legacy_opts(conf):
    """Build DEFAULT_COMMAND with 1.x include_* / generate_cpe keys applied."""
    flags = {}
    for name, default in _LEGACY_INCLUDE_DEFAULTS:
        flags[name] = _legacy_bool(conf.get(name, default), default)
    return (
        f"{DEFAULT_GENERATOR_PATH}"
        " --type cyclonedx"
        " --resultdir %(resultdir)s"
        " --root %(root)s"
        " --builddir %(builddir)s"
        f" --include-file-components {_bool_cli(flags['include_file_components'])}"
        f" --include-file-dependencies {_bool_cli(flags['include_file_dependencies'])}"
        f" --include-debug-files {_bool_cli(flags['include_debug_files'])}"
        f" --include-man-pages {_bool_cli(flags['include_man_pages'])}"
        f" --include-source-dependencies {_bool_cli(flags['include_source_dependencies'])}"
        f" --include-toolchain-dependencies {_bool_cli(flags['include_toolchain_dependencies'])}"
        f" --generate-cpe {_bool_cli(flags['generate_cpe'])}"
        " --online %(online)s"
        " --rpmbuild-networking %(rpmbuild_networking)s"
        " --isolation %(isolation)s"
        " --use-nspawn %(use_nspawn)s"
...
def resolve_generator_command(conf):
    """Return the generator argv template from plugin opts.

    Distro Mock runtime plugins get empty ``sbom_generator_opts``. Prefer an
    explicit ``command``; otherwise map leftover 1.x include_* keys onto the
    default CLI; otherwise use DEFAULT_COMMAND.
    """
    conf = conf or {}
    command = conf.get("command")
    if command:
        return command
    if any(key in conf for key in _LEGACY_INCLUDE_KEYS):
        return _command_from_legacy_opts(conf)
...
class SBOMGeneratorPlugin:
    ...
    def __init__(self, plugins, conf, buildroot):
        ...
        self.command = resolve_generator_command(self.conf)
    ...
    def _command_substitution(self):
        ...
        return {
            ...
            "online": _bool_cli(env["online"]),
            "rpmbuild_networking": _bool_cli(env["rpmbuild_networking"]),
            "isolation": str(env["isolation"]),
            "use_nspawn": _bool_cli(env["use_nspawn"]),
        }
